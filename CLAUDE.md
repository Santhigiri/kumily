...
The `guruvani` table's **data** (not just its schema) needs to be migrated out of chandiroor's database into Kumily's once Kumily is deployed — this is a one-time, separate operational step (e.g. `pg_dump`/`pg_restore` of just that table, or a `COPY` via `psql`), not something either service's code needs to handle. Until that migration runs, chandiroor's `/api/v1/guruvani/*` endpoints and Kumily's are two independent, un-synced copies of the same shape — do not treat them as interchangeable until the data has actually moved and chandiroor's guruvani feature has been removed.

---

## Known Issues and Active Work

- The `guruvani` feature is freshly migrated from chandiroor and has not yet had its underlying data moved to Kumily's own database — see "Database & Migrations" above.
